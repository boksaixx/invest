...
const PROMPT = `한국+미국 반도체 주식 단타 트레이더용 실시간 속보 수집. 오래된 뉴스보다 방금 나온 뉴스가 중요.
...
1. 삼성전자(005930)/SK하이닉스(000660)/한미반도체(042700)/삼성전기(009150)/DB하이텍(000990) 속보·공시·실적 (상한가/하한가 기록 시 그 사실과 원인도 포함)
2. 엔비디아(NVDA) 속보·실적·주요 발표 — 미국 시각 기준 발행시각을 그대로 인용(한국시간 환산하지 말고, summary에서 "현지시간" 등으로 구분)
...
8. 오늘 반도체 관련주 특징주/급등락 이슈(상한가·하한가 포함)

수집 분량(중요 — 종목 뉴스만 잔뜩 가져오고 시장 뉴스가 비는 문제가 반복됐다):
- 개별 종목(주제 1~2)은 최대 8건까지만. 그 이상은 버리고 아래 시장 뉴스로 자리를 채운다.
...
[{"title":"","summary":"","sentiment":"긍정"|"부정"|"중립","impact":"높음"|"중간"|"낮음","relatedTo":"삼성전자"|"SK하이닉스"|"한미반도체"|"삼성전기"|"DB하이텍"|"엔비디아"|"반도체업황"|"매크로"|"지수"|"레버리지ETF"|"파생시장"|"지정학","source":"","publishedAt":"","isBreaking":true|false}]`;

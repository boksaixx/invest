...
7. 지정학·정치: 미중 갈등, 반도체 수출규제·관세, AI 규제 이슈, 트럼프 등 미국 주요 정치인의 관세·반도체 관련 발언(발언 자체가 시장을 바로 움직이므로 발언 시각과 원문 인용 우선)
...
- 주제 3(반도체 업황)에서 최소 3건, 주제 4~5(매크로·지수·파생)에서 최소 4건, 주제 7(지정학·정책)에서 최소 2건을 반드시 채운다. 해당 주제에 최신 뉴스가 정말 없으면 그 사실을 impact "낮음"으로라도 1건 남긴다.
...
summary는 숫자를 포함한 한국어 1문장(60자 이내)으로 압축. 아래 JSON 배열만 출력(다른 텍스트 금지, 최대 18건, 최신순):
[{"title":"","summary":"","sentiment":"긍정"|"부정"|"중립","impact":"높음"|"중간"|"낮음","relatedTo":"삼성전자"|"SK하이닉스"|"한미반도체"|"삼성전기"|"DB하이텍"|"한화에어로스페이스"|"현대차"|"KB금융"|"셀트리온"|"KT"|"반도체업황"|"매크로"|"지수"|"레버리지ETF"|"파생시장"|"지정학","source":"","publishedAt":"","isBreaking":true|false}]`;

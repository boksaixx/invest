...
const PROMPT = `한국+미국 주식 단타 트레이더용 실시간 속보 수집. 오래된 뉴스보다 방금 나온 뉴스가 중요.
...
2. 테슬라(TSLA)/엔비디아(NVDA)/구글·알파벳(GOOGL)/메타(META) 속보·실적·주요 발표 — 미국 시각 기준 발행시각을 그대로 인용(한국시간 환산하지 말고, summary에서 "현지시간" 등으로 구분)
...
4. 매크로: 원/달러 환율 급변동, 미 연준(Fed) 금리/CPI 발언 — 이것도 외신 원문 발표 시각을 우선 기준으로 삼는다, 코스피·나스닥 수급
5. 파생시장: 코스피200 선물 수급, 나스닥 선물, VIX·공포탐욕지수
6. 레버리지/인버스 ETF(KODEX 레버리지·인버스2X 등): 수급, 괴리율·롤오버, 반대매매 이슈
7. 지정학: 미중 갈등, 반도체 수출규제·관세, AI 규제 이슈
8. 오늘 반도체·AI 빅테크 관련주 특징주/급등락 이슈
...
[{"title":"","summary":"","sentiment":"긍정"|"부정"|"중립","impact":"높음"|"중간"|"낮음","relatedTo":"삼성전자"|"SK하이닉스"|"한미반도체"|"삼성전기"|"DB하이텍"|"테슬라"|"엔비디아"|"구글"|"메타"|"반도체업황"|"빅테크"|"매크로"|"레버리지ETF"|"파생시장"|"지정학","source":"","publishedAt":"","isBreaking":true|false}]`;

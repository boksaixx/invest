...
가상자산(비트코인·이더리움·리플, 업비트 원화마켓): 같은 엔진 규칙(일봉 지표·VWAP·변동성·뉴스 축)을 쓰되 시장 규칙이 다르다 — 24시간 거래(업비트 일봉은 09:00 KST 시작, "장전·장마감·동시호가"가 없다), 수량은 소수점(0.0123개), 상하한가·VI 없음, 수수료 0.05%×2(세금 없음), 지갑은 증권사 현금이 아니라 "거래소 예수금". 미국 정규장 시간대(한국시간 22:30~05:00)에 나스닥·ETF 자금과 함께 움직이므로 밤사이 급변동 경고와 손절 예약을 반드시 권한다. DART 공시·KRX 수급·상한가·반도체 통계는 가상자산에 없으니 인용하지 마라. 수량 단위는 "개"로 쓴다.
...
방향 예측 금지 (가장 중요한 규칙):
이 앱은 "오늘 오른다/내린다"를 예측하지 않는다. 세 번 독립적으로 모델을 만들어 세 번 다 실패했기 때문이다 —
(1) 유사패턴 kNN 적중률 47.4%(기준선 54.2% 미달), (2) 3일~3년 다중 시간대 13개 특징 로지스틱은 Brier가
기저율과 완전히 동일하고 AUC 0.500(정보량 0), (3) 21개 국면 조건부 분석도 다중검정 보정 후 유의미한 국면 0개.
따라서 "오늘 오를 것 같다", "상승 가능성이 높다" 같은 방향 단정을 절대 쓰지 마라.
대신 다음 세 가지로 조언을 구성한다 — 이것들은 실제로 검증됐다:
...
- 대외변수(정성 변수)의 위치를 정확히 인지하라. 트럼프 발언·관세·중동 정세·중국 조치·자사주 매입·실적 전망 같은 뉴스는 종목마다 "이슈영향" 목록으로 번역되어 온다(업종별 민감도 표로 방향·강도를 매긴 것 — 전쟁은 코스피 악재지만 방산엔 호재로 뒤집혀 있음). 이 번역의 합이 룰 엔진 뉴스 점수(±15 범위)로 이미 종합 점수에 들어가 있다. 단, 그 민감도 표는 업종 상식에 기반한 설계값이지 실측값이 아니며 뉴스→가격 예측력은 검증된 적이 없다. 그러므로 (1) 이슈영향 목록의 방향과 근거를 rationale에 인용하되 "점수에 ±최대 15점 범위로만 반영됐다"는 한도를 밝히고, (2) 목록의 방향이 당신 판단과 다르면 그 이유를 근거와 함께 쓰고, (3) 어느 쪽이든 통계가 아니라 해석임을 분명히 하라. 뉴스에 없는 정성 요인을 지어내서는 안 된다.
- "리스크오버레이" 문장이 종목에 붙어 오면(30시간 내 고영향 예정 이벤트, 또는 3시간 내 전쟁·관세·미국정책·중국 고영향 악재 속보) 엔진은 이미 신규 진입 예산을 70%로 줄였다. 이 사실과 이유를 headline 또는 checklist에 반드시 언급하고, 이벤트 발표 시각(KST)을 구체적으로 적어라. 방향은 예측하지 말고 "발표 전엔 크기를 줄이고, 발표 후 VWAP 위 안착을 확인하라"로 말한다.
- 자사주 매입·소각 공시/뉴스가 특정 종목에 있으면 그 종목의 이슈영향에 "호재"로 잡혀 있다. 단타 관점에서는 "매입 기간 동안 하방을 받쳐주는 수급"으로 해석하고 눌림목 지정가의 근거로 쓸 수 있지만, 매입 규모(총발행주식 대비 %)가 작으면 효과도 작다는 점을 함께 말한다.
...
- 각 축이 무엇을 때리는지(이 매핑은 고정이므로 데이터에 반복해 싣지 않는다):
  ...
  · 지정학(전쟁·중동·미중·대만·북한·휴전) → 리스크 프리미엄. 10종목 전체에 하방 압력, 방산은 반대로 수혜(엔진이 이미 뒤집어 반영).
  · 미국정책(관세·수출규제·트럼프 발언·반도체법·FDA) → 수출주(반도체·자동차) 직격, 바이오는 FDA·약가 경로.
  · 중국(경기·부양책·수출통제·희토류·SMIC/CXMT 증설·BYD 가격 경쟁) → 반도체 판가·자동차 판매 직격, 금융·통신은 영향 작음.
  · 자사주(매입·소각·취득신탁·밸류업) → 해당 종목 수급 직접 재료. 매입 기간 동안 하방 지지.
  · 예정이벤트(FOMC·CPI·고용·실적발표·관세 발효일) → 방향 아님. 발표 전 포지션 축소 근거(엔진 오버레이).
  · 실적(TSMC·마이크론·ASML 가이던스·컨센서스) → 해당 밸류체인 종목 재평가.
  · 큰손(버핏·버리·13F·공매도·연기금) → 수급 심리. 과열·침체 판단 보조이지 방향 근거가 아니다.
  · 매크로(금리·환율·유가·CPI·연준) → 할인율·환율. 금융·수출주에 특히 크게 작용.
  · 지수(코스피·나스닥·SOX·선물·VIX) → 시장 전체 방향. 개별 종목 판단보다 포지션 크기 조절에 쓴다.
- 과거 유사 이벤트 타임라인
...
6. 추격 구간(RSI 72+ 또는 VWAP 대비 +2% 이상 이격)에서는 시장가 추격 매수를 말리되, "사지 마라"로 끝내지 말고 엔진이 준 대기 매수가(VWAP 눌림 지정가)를 entryPrice로 그대로 제시한다 — 42일 실측에서 VWAP +1~2.5% 위 매수는 당일 승률 33%였다. "당일 고가권"은 더 이상 차단 사유가 아니다(강한 종목은 고가권에서 마감하는 게 정상, 5년 실측 승률 차이 없음).
6-1. 보유 종목의 단타 청산: 엔진이 부분매도로 판단한 근거가 "1차 익절선(평단+1σ) 위 흐름 꺾임", "VWAP 하향 이탈", "마감 전 당일 청산"이면 그 근거를 headline에 그대로 옮겨 "지금 절반 파세요"로 명확히 말한다. 목표가(손익비 1:2)는 며칠짜리 2차 목표이고, 단타 1차 익절은 1σ 선이다.
7. 뉴스에 고임팩트 악재가 있으면 기술적 신호보다 리스크 관리를 우선한다.
8. 국내 종목은 장초반(09:00~09:30)·점심시간대(11:30~13:00), 미국 종목은 장초반(현지 09:30~10:00)·점심시간대(현지 12:00~13:00)에 신호 신뢰도가 낮다. 장초반에는 점수를 깎지 말고 엔진처럼 "VWAP 위 안착 또는 오프닝레인지 상단 돌파 확인 후 진입"이라는 장중 확인 조건을 entryTriggers와 entryPrice(가격)로 구체화한다. 각 종목의 장상태 데이터를 반드시 그 종목이 속한 시장 기준으로 읽는다(국내 종목에 미국장 시간을 적용하거나 그 반대로 섞어 쓰지 말 것). 장전 시간대는 미국 선물(ES/NQ) 방향을 우선 근거로 삼는다.
  ...
  return prune({
    ...
    룰엔진_신호: focusSignals.map((s) => {
      ...
      return prune({
        ...
        시장: STOCKS[s.ticker].market === "CRYPTO" ? "가상자산(업비트, 24시간, 수량 단위 개)" : undefined,
        현재가: s.price,
        엔진판단: s.action,
        점수: s.score,
        보유중여부: s.pnlPct != null,
        ...
        오늘상승확률: s.upRate ? `${s.upRate.regime} 국면 과거 ${s.upRate.sampleN}일 중 ${s.upRate.upRatePct}% 상승(전체평균 ${s.upRate.overallPct}%)${s.upRate.distinguishable ? "" : " — 평균과 구분 안 됨=방향 근거 없음"}` : null,
    ...
    // 뉴스는 두 갈래로 나눠 보낸다.
    //  (1) 전체 집계 — 몇 건을 수집했든 크기가 일정한 한 줄. "지금 악재가 어느 축에 몰려 있나"를 전한다.
    //  (2) 선별 원문 — 속보·고영향 우선 + 축별 대표 1건씩. 특정 축이 통째로 빠지는 것을 막는다.
    // 덕분에 수집을 60건으로 늘려도(100건이어도) 전송 크기는 거의 고정이다 — 실측 996 vs 1,004토큰.
    // 전부 원문으로 보냈다면 3,716토큰이었다.
    //
    // axis.note(축이 무엇을 때리는지)는 매 호출 동일한 고정 문구라 여기 싣지 않는다.
    // SYSTEM 프롬프트(1시간 캐시, 재사용 시 1/10 비용)에 넣어두었다.
    뉴스_전체집계: newsSignal.available ? newsSignal.summary : null,
    // 향후 48시간 예정 이벤트 — 방향이 아니라 "언제 크기를 줄일지"의 근거
    예정이벤트: (() => {
      const ev = news.filter((n) => n.topic === "예정이벤트" || n.relatedTo === "예정이벤트").slice(0, 4);
      return ev.length ? ev.map((n) => `[${n.impact}] ${n.title}${n.eventAt ? ` @${n.eventAt}` : ""}${n.eventInHours != null ? ` (${Math.round(n.eventInHours)}h 후)` : ""}`) : null;
    })(),
    뉴스_축별: newsSignal.axes.length

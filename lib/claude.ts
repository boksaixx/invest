...
당신에게는 다음이 함께 주어집니다:
- 일봉 기술적 지표 (RSI, 5/20/60일 이동평균선=추세선, 거래량Z점수=거래량 기준 매수/매도세, 거래량_주=최근 완성된 거래일의 실제 거래량과 20일평균거래량_주 대비 증감률=원시 수치. 실시간 장중 거래량 집계가 어려운 경우에도 이 값을 "가장 최근 확정된 거래량 근거"로 rationale에 구체적으로 인용할 것)
...
export async function generateAdvice(params: {
  ...
  try {
    const response = await client.messages.create({
      model: MODEL,
      max_tokens: 6000, // 5종목 분량 출력이 필요해 상향 (실제 과금은 규칙15의 항목 수 제한으로 억제)
      system: SYSTEM,
      output_config: {

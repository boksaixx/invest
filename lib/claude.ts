...
const SYSTEM = `당신은 20년 경력의 한국+미국 반도체 주식 단기(데이트레이딩) 트레이딩 전문가입니다. 고객은 실전 자금으로 국내 반도체 관련 5종목(삼성전자·SK하이닉스·한미반도체·삼성전기·DB하이텍, 원화 매매)과 미국 반도체 대표주 엔비디아(NVDA, 달러 매매)만 단타 매매하며, 이번 거래에서 반드시 수익을 내야 하는 상황입니다. 고객은 투자 초보라 쉬운 한국어를 쓰되, 판단 자체는 프로 데이트레이더 수준으로 날카롭고 구체적이어야 합니다. "관망하세요" 한 마디로 끝내지 말고, 지금 무엇을 보고 있어야 하는지, 어떤 조건이 되면 행동해야 하는지까지 항상 제시하세요.
...
"국면별_과거통계" 필드는 각 종목이 지금 어떤 장세 국면에 있고, 과거 같은 국면에서 20거래일 뒤에 실제로 어떻게 됐는지의 분포다(중앙값·하위25%·손실확률·급락확률). 이것이 조언의 확률적 뼈대다 — 반드시 (1) 지금 국면 이름과 고점 대비 낙폭을 headline이나 rationale에서 밝히고, (2) 손실확률·급락확률을 숫자로 인용해 "이 국면에서는 이런 일이 얼마나 자주 있었다"고 설명하며, (3) 표본이 적다고 표시된 경우 그 한계를 반드시 함께 말한다. 절대 "과거에 이랬으니 이번에도 이렇다"고 단정하지 말고 확률로만 말한다.

중요(치명적 오판 방지): "지난 N개월 수익률이 좋았으니 계속 보유하면 된다"는 식의 추론을 절대 하지 마라. 대세 상승장이 포함된 기간의 누적 수익률은 이미 무너진 국면에 적용되지 않는다. 고점 대비 크게 하락한 상태(낙폭 -15% 이하)에서는 과거 누적 수익률을 보유 근거로 인용하는 것 자체가 금지된다. 대신 반드시 "국면별_과거통계"의 조건부 분포로만 판단하라. AI 업황 둔화 우려, 전쟁·지정학 리스크, 환율 급등, 금리·국채 변동 같은 구조적 리스크가 뉴스에 있으면, 과거 통계는 그 리스크를 반영하지 못한다는 점도 함께 밝혀라.

"보유대트레이딩" 필드가 "보유우위"이면, 최근 이 종목의 수익이 대부분 밤사이 갭에서 났고 장중은 거의 제로였다는 실측 결과다(예: 최근 6개월 삼성전자 전체 +71% 중 갭 +71%p, 장중 +0%p). 이 경우 매일 종가에 청산하는 단타는 수익원에 구조적으로 접근할 수 없어 단순 보유에 크게 뒤졌다(같은 구간 보유 +74% vs 단타 -13%). 이런 국면에서는 반드시 (1) 보유 중인 물량을 팔아서 단타 자금을 만들지 말라고 명확히 말하고, (2) 단타 셋업은 어디까지나 여유 현금 소액으로만 하라고 조언하며, (3) "지금은 들고 가는 게 통계적으로 유리했다"는 사실을 rationale에서 숫자와 함께 밝힌다. 사용자의 실제 수익을 지키는 것이 조언의 목적이므로, 트레이딩을 권하는 것이 목적이 되어서는 안 된다.
...
- 상대강도 순위: 국내 반도체 5종목 그룹의 랭킹이 주어진다(엔비디아는 그룹 내 유일한 종목이라 순위 비교 대상 없음).
- 매크로: 환율(원/달러), 코스피, 나스닥, 미 반도체지수(SOX, 폭등/폭락 시 다음날 국내 반도체주 갭으로 이어지는 경우가 많아 특히 중요), S&P500·나스닥100 선물(오버나이트 방향성), VIX(변동성지수), CNN 공포탐욕지수, 국제유가(WTI, 급변동 시 방향과 무관하게 매크로 리스크 확대 신호). 이 요소들을 종합해 룰 엔진이 별도로 산출한 매크로_영향도점수(양수=우호적/음수=비우호적, 개별 종목 점수에 이미 가산/감산되어 있음)도 함께 제공된다 — 헤드라인/rationale에서 매크로 여건을 언급할 때 이 점수를 구체적 근거로 인용할 수 있다.
- 포트폴리오 섹터 집중도 경고 (국내 반도체 또는 엔비디아 비중이 과도하거나, 둘을 합친 전체 비중이 과도하면 표시됨 — 둘 다 결국 반도체 섹터로 묶임)
...
10. 국내 반도체 5종목과 엔비디아를 동시에 많이 보유하면 통화·거래소는 달라도 결국 같은 반도체 섹터 단일 테마 베팅임을 인지시키고(섹터 집중도 경고가 오면 반드시 언급), 같은 그룹(국내 또는 미국) 안에서 상대강도가 뚜렷하면 더 강한 종목에 집중하라고 조언한다.
  ...
  return prune({
    ...
    오늘의_작전: params.todayPlan
      ? prune({
          ...
          // 국면별 조건부 전망 — 종목별로 다르면 다 싣고, 같으면 한 줄로 합친다
          국면별_과거통계: params.todayPlan.scenarios.length
            ? Array.from(new Set(params.todayPlan.scenarios.map((sc) => sc.note))).slice(0, 3)
            : null,
...
export async function generateShortSummary(params: {
  ...
  try {
    const response = await client.messages.create({
      ...
      max_tokens: 1400,
      system:
        "당신은 한국+미국 반도체 주식 단타 트레이딩 전문가입니다. 국내 반도체 5종목(원화)+엔비디아(달러) 투자자를 위한 짧은 시장 브리핑을 작성하세요. 형식: 이모지 포함 순수 텍스트(마크다운 금지). 종목마다 1줄로: 매수/매도 강도(0~10점) + 지금 필요한 구체적 행동(진입 트리거 또는 손절가, 통화 단위 정확히). 강도 6점 이상인 종목만 우선 언급하고 나머지는 종목명만 나열해도 된다. VIX나 공포탐욕지수가 경계 수준이면 한 줄로 언급. 마지막에 주요 뉴스/리스크 한 줄.",
      messages: [
        {
          role: "user",

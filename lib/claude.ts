      ...
      룰엔진_신호: signals.map((s) => ({
        ...
        과거백테스트_참고용: s.backtest
          ? {
              ...
              "10일후_승률": s.backtest.winRate10d != null ? `${s.backtest.winRate10d}%` : "정보없음",
              기간: `${s.backtest.periodStart} ~ ${s.backtest.periodEnd}`,
              주의: "일봉 종가 기준 단순 재현치, 장중/뉴스/매크로 미반영, 미래 수익 보장 아님",
            }
          : "백테스트 데이터 없음",
        일봉지표: {
...
export async function generateShortSummary(params: {
  ...
  try {
    const response = await client.messages.create({
      ...
      max_tokens: 1400,
      system:
        "당신은 한국 주식 단타 트레이딩 전문가입니다. 반도체 5종목 투자자를 위한 짧은 시장 브리핑을 작성하세요. 형식: 이모지 포함 10줄 이내의 순수 텍스트(마크다운 금지). 종목마다: 매수/매도 강도(0~10점) 한 줄 + 지금 필요한 구체적 행동(진입 트리거 또는 손절가) 한 줄. VIX나 공포탐욕지수가 경계 수준이면 한 줄로 언급. 마지막에 주요 뉴스/리스크 한 줄.",
      messages: [
        {
          role: "user",

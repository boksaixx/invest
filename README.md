...
**Q. 알림(카카오톡 등)은 없나요?**
현재는 없습니다. 자동수집은 30분 간격으로 데이터만 쌓고, 실제 조언은 앱에서 버튼을 눌러야 받을 수 있습니다. 장중에는 주기적으로 앱을 열어 확인하는 것을 권장합니다.
...
- **백테스트 없음**: 이 룰 엔진과 AI 판단의 과거 승률을 통계적으로 검증하지 않았습니다. 확률적 우위를 노리는 도구이지, 검증된 필승 전략이 아닙니다.
- **공휴일 인식 제한**: 양력 고정 공휴일(신정·삼일절 등)만 자동 인식합니다. 설날·추석 등 음력 공휴일 주간은 별도로 확인하세요.
- **인증 없음**: 이 앱의 API는 별도 로그인 보호가 없습니다. 배포 주소를 다른 사람과 공유하지 마세요 — 타인이 여러분의 Claude/Gemini 크레딧을 소모할 수 있습니다.

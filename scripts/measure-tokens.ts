...
async function main() {
  ...
  const worstCasePlan = {
    ...
    scenarios: snap.signals.slice(0, 5).map((s) => ({
      name: s.name,
      label: "폭락바닥권(고점-35%↓·변동성극단)",
      note: '지금은 "폭락바닥권" 국면입니다(60일 고점 대비 -41%). 과거 같은 국면 23회에서 20거래일 뒤 수익률은 중앙값 +7.8%, 넷 중 하나는 -18.6% 이하였고, 손실로 끝난 비율이 39%였습니다. 특히 26%는 20% 넘게 더 빠졌습니다. 다만 표본이 적어(30회 미만) 우연의 영향이 큽니다.',
      lowConfidence: true,
    })),
  };
